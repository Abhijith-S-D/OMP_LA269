...
for N_id=1:size(N_arr,2)
    ...
    for M=M_arr
        for s_max = S_arr
            ...
            [norm_error_plot(M_id,S_id),prob_plot(M_id,S_id)] = experiment_noiseless(N,M,s_max,sigma,threshold,noise,T);
        end
    end
...
function [avg_norm_error,avg_prob] = experiment_noiseless(N,M,s_max,sigma,threshold,noise,T)
    avg_norm_error = 0;
    avg_prob = 0;
    parfor i = 1:T
        [norm_error,prob] = monteCarlo_noiseless(N,M,s_max,sigma,threshold,noise);
...
function [norm_error,prob] = monteCarlo_noiseless(N,M,s_max,sigma,threshold,noise)
    if s_max > M
        norm_error = 1;
        prob = 0;
    else
        ...
        x_hat = OMP_noiseless(A,y,s_max,threshold);
        norm_error = normalizedError(x,x_hat);
        prob = compareSupport(x,x_hat);
    end
end

function x_hat = OMP_noiseless(A,y,s_max,threshold)
    [M,N] = size(A);
    if M ~= size(y)
...
function prob_error = compareSupport(x,y)
    prob_error = isempty(setdiff(find((x~=0)==1),find((y~=0)==1)));
end
