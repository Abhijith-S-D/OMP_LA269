    ...
    for N_id=1:size(N_arr,2)
        ...
        for M=M_arr
            for s_max = S_arr
                ...
                [norm_error_plot(M_id,S_id),prob_plot(M_id,S_id)] = experiment_noise1(N,M,s_max,sigma,threshold,noise,T);
            end
        end
...
function [avg_norm_error,avg_prob] = experiment_noise1(N,M,s_max,sigma,threshold,noise,T)
    avg_norm_error = 0;
    avg_prob = 0;
    parfor i = 1:T
        [norm_error,prob] = monteCarlo_noise1(N,M,s_max,sigma,threshold,noise);
...
function [norm_error,prob] = monteCarlo_noise1(N,M,s_max,sigma,threshold,noise)
    if s_max > M
        norm_error = 1;
        prob = 0;
    else
